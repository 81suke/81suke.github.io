...
\subsubsection{全体集合を$U$,整数,自然数の集合をそれぞれ$\mathbb{Z}, \mathbb{N}$とし，奇数，偶数の集合をそれぞれ$N_o$,$N_e$とする．このとき，$N_o \cup N_e$ \boxed{\text{(1)}} , $N_o \cap N_e$ \boxed{\text{(2)}} である．また4の倍数の集合を$N_q$とすると，$N_e$ \boxed{\text{(3)}} $N_q$である．}
\begin{enumerate}[(1)]
